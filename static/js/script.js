  ...
  testCases.forEach(test => {
    ...
    div.innerHTML = `
      <div class="test-case-header">
        <span class="validation ${test.status === 'failed' ? 'failed' : ''}">${test.status === 'failed' ? '⚠ ' : ''}${test.validation}</span>
...
function createNewTest() {
  ...
  document.getElementById('newPromptInput').value = ''; // Clear input on open
}
...
function saveNewPrompt() {
  ...
  const newTest = {
    ...
    username: `#user${testCases.length + 1}`,
    prompt: newPrompt,
    status: 'pending'
  };
